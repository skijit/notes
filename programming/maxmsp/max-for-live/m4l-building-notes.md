M4L Building Examples
===========
- Notes based on built-in M4L lesson patches in Live 9

## Processing Midi Notes Lesson
- `flush` will generate note-off messages for all notes which are routed through it.
- `kslider` shows you a musical keyboard and echos whatever values it gets
- `panel` is how you can circle something or highlight it
- `midiselect` lets you specify an attribute (as paramter or in inspector) which says what kind of midi messages you want (bend, ch, ctl, pgm, note, poly).  
	- **ex**: `midiselct @note all`
		- selects all the midi *note* messages
		- the last output will send out all the midi messages which were not selected.
- `live.text` in button mode will send a bang.  
	- good for connecting it to a midiflush object, for example.  
	- if you connect to the right outlet, it will bang out the symbol message it displays... so essentially, like a message box.  
	- there are all sorts of ways to customize this image as well.
- `stripnote` and `makenote` 
	- to manage note-offs you often want to regenerate the midi note **BUT** have the patch control the note off messages.  so, you use a stripnote object to only pass note-on messages, then a makenote object which will generate the corresponding note-off messages (based on some input duration).
- midi output objects:
	- `midiout` (by default, this is in m4l midi devices)
	- `noteout` (sends note data to live)
- `translate` can convert notevalues (durations) into ms
...
- `tapin~` and `tapout~` correspond to each other and you set a parameter for max buffer size on the `tapin~` and a delay amount in the `tapout~`.
	- bc they are necessarily correspond with each other, its a control patch cord which connects them.
- you can use a signal to set the delay time
- dry/wet(2) is a good example of how to do a single dry/wet dial.  you should use multiply the resulting values by the `line~` with a ramp time of 50ms to avoid clicks.
...
- `translate` will also output a bang if the global tempo changes  (use case translate notevalues (durations) ms)
- `sah~` is the sample and hold object.  
	- it has 2 inlets: 
		1. one for a main signal which has the values which get passed on 
		2. one for another signal that when it crosses a specified threshold will trigger output of the main signal value.
- the `number~` can be used to convert a number into a signal, which can be useful for certain programming strategies
- `scope~` creates a oscilloscope for a signal you pass in to it.
- you can use `live.tab` to select from a bank of options- like various time divisions
- `filtergraph~` lets you graphically build filter coefficients, which will be entered into the `biquad~`
- `pfft~` performs a windowed DFT and gives these results to the specified subpatcher (argument 1).  
	- other arguments for `pfft~` involve the window size and hop size.
...
- `gizmo~` performs freq/pitch shifting in freq-domain.  use this inside your `pfft~` subpatch
...
- FM basics:
	...
	- **modulation signal**: has a rate and depth setting.  
		- rate sets the modulation freq 
		- depth is the amplitude
...
- `preset` is how you can store presets inside a patcher 
	- or in a separate preset file- see `pattr` / presets notes below
...
- a lot of times you need to use `number~` to convert a number into a signal

## Envelopes
- `live.text` in button mode will not only display text, but output a bang
- `live.numbox` has 3 styles: normal, slider, triangle
	- `function` lets you draw an envelope.  
	- you can then use this as an input to a `line~` to generate the envelope.
...
## Adding in a MIDI Note
- `adsr~` is an envelope generator.  a,d,r stages are milliseconds and s is in amplitude.  s stage is triggered when an amplitude of 0 is reached.  
	- it can't handle polyphony
	- actually, an overlapping note will not get it's own attack with this scheme.
- `notein` captures midi input, and `stripnote` will filter out the note off messages.
...
## Adding Polyphony
- `poly~` lets you add polyphony by adding copies of the same subpatch (abstraction), but all using the same object. 
	- the second argument to `poly~` is how many instances/voices you want.
	- `poly~` uses `in`, `in~`, `out`, and `out~` objects instead of the typical subpatcher io objects, inlet and outlet.
- inside the `poly~`, the `thispoly~` is used for managing voices:
	- `poly~` has to decide which instance to use for a note.  
	- `thispoly~` accepts 'mute 1' (=voice is in use) or 'mute 0' (voice is available) messages, from `adsr~` which tells `poly~` whether to use this instance or not.
	- a new midi note is converted to a message: 'midinote <note> <velocity>', so that `poly~` understands a new note is being submitted.
- the leftmost inlet for a `poly~` is the only inlet that accepts messages targeted to a particular instance.  all other inlets correspond to all instances.
...
- `zl.reg` stores the envelope data- then every time it gets banged (corresponding to a new midi note, it will output this data to the `line~`, generating an envelope)
- `sig~` converts a number to a signal.  
	- big difference between `sig~` and `number~`, I think, is that sig~ will output the signal through its outlet whereas `number~` will not.
- how to tell whether a patcher is loaded as an encapsulation or an abstraction: check the bottom, left hand corner of the patcher.  if it has the 'modify read-only' button enabled, it was loaded as an abstraction.
...
- vibrato in the extreme -> FM, and autopanning in the extreme -> ring modulation
	- we get an autopanning effect from using tremolo (amplitude modulation) b/c the `freqshift~` we're using has 2 outlets- one for pos, one for neg sidebands, which are separated into l/r outputs for stereo widening.

## Randomization
- `decide` lets you choose randomly between 0 and 1
- you can use a decision process with cascading decide -> select 0 1 statements

## MIDI Generation
- `makenote`: specify a note, velocity, and duration and it will generate the note-on and note-off messages
...
- `live.step`:  you can change step sizes, loop regions, and interval updates
	- you can fold it to see only the notes, etc you're using        
	- there are a number of messages you can send it        	
...
- `itable` can store probability weights and if you send it a bang, it will output one of it's indexes based on the weight stored in it
- `funnel` takes any number of inlets (spec'ed as parameter) and will pass that data through along with the index value (inlet number) prefixed to the outputt'ed value
- `metro` and `counter` can work together for timing-related events which are synchronized to the live transport
- you can have many different metros in a patch and they can do many things beyond just creating notes!

## MIDI Note Operations
- `trigger` can also send a message.  
- floating windows:
	- based on creating a new presentation view in a subpatcher
	- involves `pcontrol` and `thispatcher`
	- `pcontrol` is attached to a patcher object (encapsulation or abstraction) in the parent, and you'll send it an 'open' message.
	- `thispatcher` needs to recieve a bunch of 'window ...' messages for sizing and setting window behavior.  see help for info.
- **M4L.api.GetSelectedNotes**:
	- This is a helper abstraction to load the notes from the selected clip into a jitter matrix.
	- First, send a message: 'path live_set view detail_clip' into a `live.path`.  This navigates to the clip currently displayed in the clip view.
	- The `live.path` outputs the live object's id and this will get passed to a `live.object`.  
	...
	- After you call a method on a `live.object`, you'll want to capture that output with a `route`:
		- `route <methodname>`
		That is bc the arguments to route will try to match the first item in any message sent to it, and presumably the first item in the output from `live.object`'s invocation of this method is the name of the method.
	- `zl` performs a variety of list processing. The type of processing depends on the argument.
		...
		- `sort`:  ...
		- `sum`:   ...
- jitter matrix: good for holding note values `jit.matrix`
...
- Calling methods against a `live.object`:
	...
	- example- Clip.set_notes() looks like this:
		...
		call done``` 
- `jit.iter` lets you iterate through a jitter matrix- sending a message for each cell.
...
- 'device id 0' is a way of freeing up a `live.object` or `live.remote` to no longer be bound to a real live object.
...
- `deferlow`  defers all messages to the tail of the low priority queue
- `defer`: defers all messages to the head of the low priority queue.  
	- This is useful in overdrive mode!
- `substitute`: you specify two arguments and if a message passing through `substitute` contains some element matching the first argument, it is replaced with the second argument.  
