...
- [another good source](http://www.resistorguide.com/pull-up-resistor_pull-down-resistor/)
- [one more](http://forum.allaboutcircuits.com/threads/floating-vs-grounded.10670/)
- [this](http://www.learningaboutelectronics.com/Articles/Why-does-a-circuit-always-have-to-have-ground)
- [this is the best!](https://www.youtube.com/watch?v=wxjerCHCEMg)
- Digital circuits can have 1 of 3 voltage states:
    1. ON (Has Voltage)
    2. OFF (Voltage is ground)
    3. Disconnected (Voltage is floating)

- Imagine a 3-State Gate which has the following states:
    
- The Disconnected state is a problem in many circumstances:
    - Imagine a circuit: Voltage Source (5V) -- Switch --- Microcontroller
        - When switch is closed, the input to microcontroller is 5V
        - When switch is open, the input to microcontroller it is floating
            - Must be since there's no path to ground!
            - The problem is that can lead to stray voltage which is ambiguous to the microcontroller
- The point of pull up resistors and pull down resistors are the same: to insure that given no other input, the circuit assumes an unambiguous value.
    - pull up: pulls it high
    - pull down: pulls it low
- Recall:
